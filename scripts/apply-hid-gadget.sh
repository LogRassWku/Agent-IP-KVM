...
apply_hid() {
    ...
    if [ "$INCLUDE_RELATIVE_MOUSE" -eq 1 ] && { [ -e "$GADGET/functions/$MOUSE_FUNCTION" ] || [ -e "$CONFIG/$MOUSE_FUNCTION" ]; }; then
        [ -d "$GADGET/functions/$MOUSE_FUNCTION" ] && [ -L "$CONFIG/$MOUSE_FUNCTION" ] || fail "$MOUSE_FUNCTION is partial"
        CREATE_MOUSE=0
    fi
    if [ "$INCLUDE_ABSOLUTE_POINTER" -eq 1 ] && { [ -e "$GADGET/functions/$POINTER_FUNCTION" ] || [ -e "$CONFIG/$POINTER_FUNCTION" ]; }; then
    ...
    if [ "$INCLUDE_ABSOLUTE_POINTER" -eq 0 ] && [ -e "$GADGET/functions/$POINTER_FUNCTION" ]; then
        RECREATE_POINTER=1
    fi
    ...
    if [ "$INCLUDE_ABSOLUTE_POINTER" -eq 1 ] && { [ "$CREATE_POINTER" -eq 1 ] || [ "$RECREATE_POINTER" -eq 1 ]; }; then
        mkdir "$GADGET/functions/$POINTER_FUNCTION"
    fi
    ...
    APPLY_COMPLETE=1
    trap - EXIT HUP INT TERM
    echo "Agent IP KVM HID functions are active."
}
...
status_hid() {
    wait_for_gadget
    if [ -L "$CONFIG/$KEYBOARD_FUNCTION" ] && \
       { [ "$INCLUDE_RELATIVE_MOUSE" -eq 0 ] || [ -L "$CONFIG/$MOUSE_FUNCTION" ]; } && \
       { [ "$INCLUDE_ABSOLUTE_POINTER" -eq 0 ] || [ -L "$CONFIG/$POINTER_FUNCTION" ]; }; then
        echo "active"
    else
        echo "inactive"
    fi
