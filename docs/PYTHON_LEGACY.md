...
> 以下为原 Python 版本的运行说明与实机记录，不代表 TypeScript 版本已通过硬件验证。完整原版保存在 [legacy 分支](https://github.com/LogRassWku/Agent-IP-KVM/tree/legacy)。`main` 已移除 Python 实现；本文命令均在 `legacy` 检出目录的根目录执行。

需要运行旧版时，可在项目外另建目录，避免影响当前 TypeScript 工作区：

```sh
git clone --branch legacy --single-branch https://github.com/LogRassWku/Agent-IP-KVM.git Agent-IP-KVM-legacy
cd Agent-IP-KVM-legacy
```
